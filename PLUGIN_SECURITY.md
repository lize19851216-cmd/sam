...
The local named-pipe contract is metadata-only and restricted to the current Windows user. It does not grant an untrusted plugin host services, and it is not an operating-system sandbox. A future out-of-process host must add OS-level restriction before untrusted plugin code can run.
